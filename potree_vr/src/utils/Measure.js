...
Potree.Measure = class Measure extends THREE.Object3D {
	constructor () {
		...
		this.ifz = false;
		this.normalVectorDistances = false;
		this.vectorXYZ = [];
			...
			let drag = (e) => {
				...
				if (I) {
					...
					if (i !== -1) {
						...
						if(this.normalVectorDistances&&(i === 0)){
							...
							for(let m = 0;m < 5; m++){
								vector1x[m] = I.location5[m].x - I.location.x;
								vector1y[m] = I.location5[m].y - I.location.y;
								vector1z[m] = I.location5[m].z - I.location.z;
								
							}
							
							for(let m = 0;m < 4;m++){
								vector2x[m] = (vector1y[m]*vector1z[(m + 1)]) - (vector1z[m]*vector1y[m +1]);
								vector2y[m] = (vector1z[m]*vector1x[m + 1]) - (vector1x[m]*vector1z[m +1]);
								vector2z[m] = (vector1x[m]*vector1y[m + 1]) - (vector1y[m]*vector1x[m +1]);
								
							}
							
							for(let m = 0,k = 0;m < (5-1);m += 2,k++){
								
								vector3x[k] = vector2x[m] + vector2x[m + 1];
								vector3y[k] = vector2y[m] + vector2y[m + 1];
								vector3z[k] = vector2z[m] + vector2z[m + 1];
								
							}
							
							this.vectorXYZ[0] = vector3x[0] + vector3x[1];
							this.vectorXYZ[1] = vector3y[0] + vector3y[1];
							this.vectorXYZ[2] = vector3z[0] + vector3z[1];
				
						}
						if(this.normalVectorDistances&&(i === 1)){
							this.setPosition(i,I.location,this.vectorXYZ[0],this.vectorXYZ[1],this.vectorXYZ[2]);
						}else{
