...
Potree.GeoJSONExporter = class GeoJSONExporter {
	...
	// 导出标签内容
	static anJSON (annotations) {
		let els = [];
		for(let an of annotations.children) {
			let el = {
				title: an._title,
				description: an._description,
				position: an.position.toArray(),
				cameraPosition:an.cameraPosition.toArray(),
				cameraTarget:an.cameraTarget.toArray()
			};
			els.push(el);
		}
		let anjson = {
		...
		// 返回数据
		return JSON.stringify(anjson, null, '\t');
	}
};
