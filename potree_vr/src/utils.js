...
Potree.utils = class {
	...
	//将屏幕坐标转换为世界坐标
	static getMouseWorldCoord(mouse, camera, viewer) {
		...
		//相机坐标系转换到世界坐标系
		let camPos = new THREE.Vector3(nmouse.x, nmouse.y, 0.5);
		camPos.unproject(camera);
	...
	static getMousePointCloudIntersection (mouse, camera, viewer, pointclouds, params = {}) {
		...
		if(params.pickClipped){
			...
			for(let m = 0;m < 5; m++){
				pickParams5[m].pickClipped = params.pickClipped;
			}
		}
		...
		//五个点的窗口位置
		pickParams5[0].x = mouse.x;
		pickParams5[0].y = renderer.domElement.clientHeight - (mouse.y - 10);

		pickParams5[1].x = mouse.x - 8.660254;
		pickParams5[1].y = renderer.domElement.clientHeight - (mouse.y + 5);

		pickParams5[2].x = mouse.x + 8.660254;
		pickParams5[2].y = renderer.domElement.clientHeight - (mouse.y + 5);

		pickParams5[3].x = mouse.x - 10;
		pickParams5[3].y = renderer.domElement.clientHeight - (mouse.y);

		pickParams5[4].x = mouse.x + 10;
		pickParams5[4].y = renderer.domElement.clientHeight - (mouse.y);
		...
		for(let pointcloud of pointclouds){
			...
			for(let m = 0; m < 5; m ++){
				//得到五个点在点云上的三维坐标
				points5[m] =  pointcloud.pick(viewer, camera, ray, pickParams5[m]);
			...
			if (distance < closestDistance) {
				...
				for(let m = 0; m < 5; m++){
					Intersection5[m] = points5[m].position;
				}	
			}
		}
