...
Potree.Scene = class extends THREE.EventDispatcher {
	...
	constructor() {
		...
		this.vrcontroller = new THREE.VRControls(this.cameraP);
	...
	addPathPoint(measurement) {
		measurement.lengthUnit = this.lengthUnit;
		this.pathpoints.push(measurement);
		this.dispatchEvent({
			'type': 'measurement_added',
			'scene': this,
			'measurement': measurement
	...
	removePathPoint(measurement) {
		let index = this.pathpoints.indexOf(measurement);
		if (index > -1) {
	...
	removePanorama(panorama) {
		let index = this.panoramas.indexOf(panorama);
		if (index > -1) {
			this.panoramas.splice(index, 1);
			this.dispatchEvent({
				'type': 'panorama_removed',
				'scene': this,
				'panorama': panorama
			});
		}
	...
	zoomToPano(panorama) {
		let ind = this.panoramas.indexOf(panorama);
		if (ind === this.zoomedPnaoInd) return;

		//zoom out old panorma
		for (var i = 0; i < this.panoramas.length; i++) {
			if (!this.panoramas[i].useThumbnail)
				this.panoramas[i].setuseThumbnail(true);
		}

		//zoom to new panorma
		this.panoramas[ind].setuseThumbnail(false);
	}
	...
	removeAllPathPoints() {
		...
		while (this.pathpoints.length > 0) {
			this.removePathPoint(this.pathpoints[0]);
		}
	...
	getActiveCamera() {
		...
		if(this.cameraMode ===  Potree.CameraMode.PERSPECTIVE){
			return this.cameraP;
		}else if(this.cameraMode ===  Potree.CameraMode.ORTHOGRAPHIC){
			return this.cameraO;
		}else if(this.cameraMode ===  Potree.CameraMode.VR){
